...
In the previous section, we used the inverse method from \citet{Hines2016} to constrain the effective viscosity structure required to explain the first 0.8 years of postseismic deformation. In this section, we use the effective viscosities inferred above as a prior constraint when searching for models which are capable of describing the available 5 years of postseismic data, where our forward problem is now eq. (\ref{GeneralForward}) rather than the approximation given by eq. (\ref{ApproxForward}).  We perform a series of coseismic slip and afterslip inversion assuming a variety of rheologies for the lower crust and upper mantle which are consistent with our findings from Section \ref{sec:InitialInversion}.  We appraise each model using the mean chi squared value,
...
\bibitem[{\textit{Aagaard et~al.}(2009)\textit{Aagaard, Williams, Knepley,
  Kientz, and Strand}}]{Aagaard2009}
Aagaard, B., C.~Williams, M.~Knepley, S.~Kientz, and L.~Strand (2009),
  {PyLith}.
...
Argus, D.~F., M.~B. Heflin, G.~Peltzer, F.~Cramp{\'{e}}, and F.~H. Webb (2005),
  ...
  Los Angeles}, \textit{Journal of Geophysical Research B: Solid Earth},
