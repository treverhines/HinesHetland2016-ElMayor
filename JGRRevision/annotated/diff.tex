...
%DIF DEL old.tex   Sat Jul  2 00:54:11 2016
%DIF ADD new.tex   Thu Jul  7 22:02:55 2016
\draftfalse
...
%DIFDELCMD < %%%
\DIFdelend \authors{T. T. Hines\affil{1}, E. A. Hetland\affil{1}}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \affiliation{1}{Department of Earth and Environmental Sciences, University of Michigan, Ann Arbor, Michigan, USA.}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \correspondingauthor{T. T. Hines}{hinest@umich.edu}
...
%DIFDELCMD < %%%
\DIFdelend \item Near-field postseismic deformation exhibits early transience that decays to a sustained rate which is elevated above the preseismic trend 
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \item \DIFdelbegin \DIFdel{A }\DIFdelend \DIFaddbegin \DIFadd{Far-field postseismic deformation can be explained with a }\DIFaddend Zener or Burgers \DIFdelbegin \DIFdel{upper mantle can explain far-field deformation while near-field transience can be described with afterslip
}\DIFdelend \DIFaddbegin \DIFadd{rheology upper mantle 
}\DIFaddend \end{keypoints}

\begin{abstract}
\DIFdelbegin \DIFdel{Five years of postseismic deformation following the Mw7.2 }\DIFdelend \DIFaddbegin \DIFadd{We analyze five years of Southern California GPS data following the Mw=7.2 }\DIFaddend El Mayor-Cucapah earthquake\DIFdelbegin \DIFdel{reveals transient deformation that decays back to its pre-earthquake trend after about }\DIFdelend \DIFaddbegin \DIFadd{.  We observed transient postseismic deformation which persists for }\DIFaddend three years at epicentral distances greater than ${\sim}200$ km.  \DIFdelbegin \DIFdel{At closer distances, the rapid }\DIFdelend \DIFaddbegin \DIFadd{In the near-field, rapid postseismic }\DIFaddend transience decays to a sustained rate which exceeds its \DIFdelbegin \DIFdel{pre-earthquake }\DIFdelend \DIFaddbegin \DIFadd{preseismic }\DIFaddend trend.  We attempt to determine the mechanisms driving this deformation, where we consider afterslip at seismogenic depths and viscoelastic relaxation in the lower crust and upper mantle as candidate mechanisms.  We find that early, rapid, near-field deformation can be explained with afterslip on the fault that ruptured coseismically\DIFdelbegin \DIFdel{, while the }\DIFdelend \DIFaddbegin \DIFadd{. The }\DIFaddend later, sustained, near-field deformation can be explained with \DIFdelbegin \DIFdel{either continued afterslip in an effectively elastic lower crust , or lower crustal viscoelastic relaxation }\DIFdelend \DIFaddbegin \DIFadd{viscoelastic relaxation in the lower crust }\DIFaddend with a steady-state viscosity of ${\sim}10^{19}$ Pa s \DIFdelbegin \DIFdel{. The trend in }\DIFdelend \DIFaddbegin \DIFadd{and possibly continued afterslip.  The later postseismic deformation in the }\DIFaddend far-field \DIFdelbegin \DIFdel{deformation }\DIFdelend is best explained with a transient viscosity of ${\sim}10^{18}$ Pa s in the upper mantle. We argue that a transient rheology in the mantle is preferable over a Maxwell rheology because it better predicts the decay in postseismic deformation, and also because it does not conflict with the generally higher, steady-state viscosities inferred from studies of geophysical processes occurring over longer \DIFdelbegin \DIFdel{time-scales}\DIFdelend \DIFaddbegin \DIFadd{time scales}\DIFaddend .
\end{abstract}

\section{Introduction}
Ground deformation in the years following a large (\DIFaddbegin \DIFadd{Mw}\DIFaddend $\gtrsim$\DIFdelbegin \DIFdel{Mw7) earthquake provides }\DIFdelend \DIFaddbegin \DIFadd{7) earthquake can be used to gain }\DIFaddend insight into the mechanical behavior of the crust and upper mantle. \DIFdelbegin \DIFdel{It has long been recognized that }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend interpretations of postseismic deformation \DIFdelbegin \DIFdel{can be ambiguous }\DIFdelend \DIFaddbegin \DIFadd{are not always conclusive }\DIFaddend because multiple postseismic deformation mechanisms\DIFaddbegin \DIFadd{, such as afterslip or viscoelastic relaxation in the lower crust and upper mantle, }\DIFaddend can have qualitatively similar surface expressions \citep[e.g.][]{Savage1990}. \DIFaddbegin \DIFadd{This non-uniqueness complication can potentially be remedied if the postseismic deformation occurs in an area that is sufficiently well instrumented with GPS stations \mbox{%DIFAUXCMD
\citep{Hearn2003}
}%DIFAUXCMD
.  }\DIFaddend Owing to the dense geodetic network deployed throughout the 2000s as part of the Plate Boundary Observatory, the postseismic deformation following the April 4, 2010, \DIFdelbegin \DIFdel{Mw7.2 }\DIFdelend \DIFaddbegin \DIFadd{Mw=7.2 }\DIFaddend El Mayor-Cucapah earthquake in Baja California was observed at more GPS stations than any other earthquake in California to date \DIFaddbegin \DIFadd{(see \mbox{%DIFAUXCMD
\citet{Hauksson2011}
}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citet{Fletcher2014}
}%DIFAUXCMD
for a detailed description of this earthquake and its seismotectonic context)}\DIFaddend .  With such a large collection of data, we attempt to discern the mechanisms driving \DIFdelbegin \DIFdel{postseismic deformation following the El Mayor-Cucapah earthquake, where we consider both afterslip and viscoelastic relaxation in the lower crust and upper mantle as candidate mechanisms}\DIFdelend \DIFaddbegin \DIFadd{the postseismic deformation}\DIFaddend . 
...
In this study, we perform a kinematic inversion for fault slip, allowing it to persist throughout the postseismic period, while simultaneously estimating the viscosity of the lower crust and upper mantle.  We create an initial model of the fault slip and effective viscosity necessary to describe early postseismic deformation using the method described in \citet{Hines2016}.  This method \DIFdelbegin \DIFdel{utilizes }\DIFdelend \DIFaddbegin \DIFadd{uses }\DIFaddend a first-order approximation of surface deformation resulting from viscoelastic relaxation which is only applicable to the early postseismic period. In this case, our initial model describes the first 0.8 years of postseismic deformation following the El Mayor-Cucapah earthquake.  We then use the inferred effective viscosity structure from the initial model to create a suite of postseismic models \DIFdelbegin \DIFdel{to explain }\DIFdelend \DIFaddbegin \DIFadd{which we test against }\DIFaddend the five years of postseismic data available to date.  Of the suite of models tested, we find that postseismic deformation following the El Mayor-Cucapah earthquake can be explained with a combination of afterslip on a fault segment running through the Sierra Cucapah and \DIFaddbegin \DIFadd{viscoelastic relaxation in }\DIFaddend a Zener rheology \DIFdelbegin \DIFdel{in the }\DIFdelend upper mantle with a transient viscosity \DIFdelbegin \DIFdel{that decays from $6\times10^{18}$ Pa s to $1\times10^{18}$ Pa sat 120 km depth}\DIFdelend \DIFaddbegin \DIFadd{on the order of $10^{18}$ Pa s}\DIFaddend . 

\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=0.8]{Figures/ContextMap} 
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=0.8]{Figures/2016jb013114-p01} 
\DIFaddendFL \caption{Map of the region considered in this study.  The large focal mechanism is \DIFaddbeginFL \DIFaddFL{the GCMT solution }\DIFaddendFL for the El Mayor-Cucapah earthquake, and the three small focal mechanisms are for the Ocotillo earthquake and the two main shocks during the Brawley swarm.  The black dots indicate the locations of GPS stations used in this study.  The fault geometry used in this study is shown in \DIFdelbeginFL \DIFdelFL{cyan }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{magenta }\DIFaddendFL where dashed lines indicate buried edges of the fault segments.  The green and red boxes demarcate the extent of the near-field and far-field maps (Figures \ref{fig:NearField} and \ref{fig:FarField}).  Stations inside the blue sector, which highlights the area within 10$^\circ$ of the El Mayor-Cucapah \DIFdelbeginFL \DIFdelFL{P axis}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{P-axis}\DIFaddendFL , are used in Figures \DIFdelbeginFL \DIFdelFL{\ref{fig:RecordSectionElastic}, \ref{fig:RecordSection1}, }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\ref{fig:RecordSectionMain} }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{\ref{fig:RecordSection2}}%DIFDELCMD < }%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\ref{fig:RecordSection1}}\DIFaddendFL .\DIFaddbeginFL }       
\DIFaddendFL \label{fig:ContextMap}
\end{figure}

\section{Data Processing}\label{sec:Data}
We use continuous GPS position time series provided by University Navstar Consortium (UNAVCO) for stations within a 400 km radius about the El Mayor-Cucapah epicenter. We collectively describe the coseismic and postseismic displacements resulting from the El Mayor-Cucapah earthquake as $u_\mathrm{post}(t)$.  We consider the GPS position time series\DIFaddbegin \DIFadd{, }\DIFaddend $u_\mathrm{obs}(t)$\DIFaddbegin \DIFadd{, }\DIFaddend to be the combination of $u_\mathrm{post}(t)$, secular tectonic deformation, annual and semi-annual oscillations, and coseismic offsets from significant earthquakes over the time span of this study.  The June 14, 2010, \DIFdelbegin \DIFdel{Mw5.8 }\DIFdelend \DIFaddbegin \DIFadd{Mw=5.8 }\DIFaddend Ocotillo earthquake and the Brawley swarm, which included an \DIFdelbegin \DIFdel{Mw5.5 and an Mw5.4 }\DIFdelend \DIFaddbegin \DIFadd{Mw=5.5 and an Mw=5.4 }\DIFaddend event on August 26, 2012 \DIFdelbegin \DIFdel{, }\DIFdelend (Figure \ref{fig:ContextMap})\DIFaddbegin \DIFadd{, }\DIFaddend are the only earthquakes that produced noticeable displacements in any of the time series.  We treat the displacements resulting from the Brawley swarm as a single event because the daily solutions provided by UNAVCO cannot resolve the separate events.  Although the Ocotillo earthquake had its own series of aftershocks \citep{Hauksson2011}, neither the Ocotillo earthquake nor the Brawley swarm produced detectable postseismic deformation.  We model displacements resulting from these events with only a Heaviside function, $H(t)$, describing the coseismic offsets.  We then model $u_\mathrm{obs}(t)$ as 
...
\begin{equation}\label{TimeSeriesModel}
  ...
  \end{split}
\end{equation}
In the above equations, $t_\mathrm{emc}$, $t_\mathrm{oc}$ and $t_\mathrm{bs}$ are the times of the El Mayor-Cucapah earthquake, Ocotillo earthquake, and the Brawley swarm, respectively, $c_0$ through $c_7$ are unknown coefficients, and $\epsilon$ is the observation noise.  We \DIFaddbegin \DIFadd{are using years as our unit of time which makes $c_2$ through $c_5$ the coefficients for annual and semi-annual oscillations.  We }\DIFaddend only estimate jumps associated with the Ocotillo earthquake and Brawley swarm for stations within 40 km of their epicenters. 

Stations which recorded displacements that clearly cannot be described by the aforementioned processes are not included in our analysis. This includes stations in the Los Angeles basin, where anthropogenic deformation can be larger than the postseismic signal that we are trying to estimate \citep{Bawden2001,Argus2005}. In order to ensure an accurate estimation of the secular deformation, we only use stations that were installed at least six months prior to El Mayor-Cucapah earthquake \DIFdelbegin \DIFdel{. Several }\DIFdelend \DIFaddbegin \DIFadd{even though several }\DIFaddend GPS stations were installed after the \DIFdelbegin \DIFdel{El Mayor-Cucapah earthquake to improve the spatial resolution of postseismic deformation \mbox{%DIFAUXCMD
\citep{Spinler2015}
}%DIFAUXCMD
.  Although it }\DIFdelend \DIFaddbegin \DIFadd{earthquake to get better coverage of the postseismic deformation field \mbox{%DIFAUXCMD
\citep{Spinler2015}
}%DIFAUXCMD
.  It }\DIFaddend would be possible to subtract secular velocities derived from elastic block models \citep[e.g.][]{Meade2005} from velocities recorded at the newly installed stations to get an estimate of postseismic velocities \DIFdelbegin \DIFdel{, we do not do so here. We use coseismic and postseismic displacements, rather than velocities, in our inverse method described in Section \ref{sec:Model}. We use displacements because }\DIFdelend \DIFaddbegin \DIFadd{at those stations. However, }\DIFaddend estimating velocities from an already noisy displacement time series can introduce significant uncertainties depending on exactly how the estimation is done.  \DIFaddbegin \DIFadd{We therefore use coseismic and postseismic displacements, rather than velocities, in our inverse method described in Section \ref{sec:Model}. }\DIFaddend This choice prevents us from using the newly installed stations \DIFdelbegin \DIFdel{in Baja California }\DIFdelend for our analysis.   

The October 16, 1999, \DIFdelbegin \DIFdel{Mw7.1 }\DIFdelend \DIFaddbegin \DIFadd{Mw=7.1 }\DIFaddend Hector Mine earthquake, which occurred ${\sim}270$ km north of the El Mayor-Cucapah epicenter, produced transient postseismic deformation which we do not wish to model, either mechanically or through empirical line fitting.  We thus restrict our analysis to deformation observed six years after the Hector Mine earthquake, which is when postseismic velocities at sites \DIFdelbegin \DIFdel{proximal to }\DIFdelend \DIFaddbegin \DIFadd{near }\DIFaddend the Hector Mine epicenter are approximately constant \citep{Savage2009}. When appraising our model fit in Section \ref{sec:Model}, we see some systematic residuals in the vicinity of the Hector Mine epicenter, which may be the result of errors in the assumption that the trend in Hector Mine postseismic deformation is linear after six years.   
...
The smoothness of $u_\mathrm{post}(t)$ is controlled by the chosen value of $\sigma^2$, which describes how rapidly we expect postseismic displacements to vary over time.  Setting $\sigma^2$ equal to zero will effectively result in modeling $u_\mathrm{post}(t)$ as a straight line which is insufficient to describe the expected transient behavior in postseismic deformation. The other end member, where $\sigma^2$ is infinitely large, will result in $u_\mathrm{pred}(t)$ \DIFdelbegin \DIFdel{over fitting }\DIFdelend \DIFaddbegin \DIFadd{overfitting }\DIFaddend the data. While one can use a maximum likelihood based approach for picking $\sigma^2$ \citep[e.g.][]{Segall1997}, we instead take a subjective approach and choose a value for $\sigma^2$ that is just large enough to faithfully describe the observed deformation at the most near-field station in our study, P496, which exhibits the most \DIFdelbegin \DIFdel{pronounced }\DIFdelend rapid changes in velocity. This ensures that $\sigma^2$ will be sufficiently large so that our estimate of $u_\mathrm{post}(t)$ does not smooth out potentially valuable postseismic signal at the remaining stations. We find that using $\sigma^2 = 0.05 \mathrm{m}^2 / \mathrm{yr}^3$ adequately describe all but the first week of postseismic deformation at station P496, which \DIFdelbegin \DIFdel{gets incorporated into }\DIFdelend \DIFaddbegin \DIFadd{slightly increases }\DIFaddend our estimate of coseismic displacements (Figure \ref{fig:P496}).  We \DIFdelbegin \DIFdel{assume that the first week of deformation is overwhelmingly the result of afterslip and this afterslip is lumped into our estimates of coseismic slip. \mbox{%DIFAUXCMD
\citet{Freed2007a}
}%DIFAUXCMD
noted that postseismic deformation can be observed at distances greater than 200 km from the Hector Mine earthquake and, after filtering the time series for stations up to 400 km from }\DIFdelend \DIFaddbegin \DIFadd{include an example of estimating $u_\mathrm{post}(t)$ for a far-field station, P619, which is about 359 km north of }\DIFaddend the El Mayor-Cucapah epicenter \DIFdelbegin \DIFdel{, we also clearly see far reaching postseismic transient deformation }\DIFdelend (Figure \ref{fig:P619}).  \DIFaddbegin \DIFadd{At station P619, along with all the other stations in the Mojave region, there is a south-trending postseismic transience that persists for the first three years after the El Mayor-Cucapah earthquake.  Postseismic deformation that extends to these epicentral distances has also been observed after the Hector Mine earthquake \mbox{%DIFAUXCMD
\citep{Freed2007a}
}%DIFAUXCMD
.
}\DIFaddend 
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=0.8]{Figures/FilterP496}
%DIFDELCMD < %%%
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=0.8]{Figures/FilterP619}
%DIFDELCMD < %%%
...
We show in Figures \ref{fig:NearField} and \ref{fig:FarField} the near and far-field \DIFaddbegin \DIFadd{coseismic displacements and the }\DIFaddend postseismic displacements accumulated over the time intervals 0-1 years, 1-3 years, and 3-5 years\DIFdelbegin \DIFdel{, as well as the coseismic displacements}\DIFdelend .  Stations at epicentral distances beyond ${\sim}200$ km have an elevated rate of deformation for the first three years following the earthquake.  This far-field deformation \DIFdelbegin \DIFdel{has a southward trend }\DIFdelend \DIFaddbegin \DIFadd{is trending southward at a rate of a few millimeters per year }\DIFaddend along the direction of the El Mayor-Cucapah \DIFdelbegin \DIFdel{P axis and a }\DIFdelend \DIFaddbegin \DIFadd{P-axis.  A }\DIFaddend similar eastward trend can be seen in the few far-field stations in Arizona, located along the \DIFdelbegin \DIFdel{T axis}\DIFdelend \DIFaddbegin \DIFadd{T-axis}\DIFaddend .  After three years, the trend in far-field postseismic deformation is barely perceptible.  Most far-field stations display an initial subsidence for the first year after the El Mayor-Cucapah earthquake followed by continued uplift.  This trend in vertical deformation can be observed in all three of the quadrants where postseismic data is available, which means that the vertical deformation does not exhibit an anti-symmetric quadrant pattern, as would be expected for postseismic processes.  Although we use vertical deformation in our analysis in Section \ref{sec:Model},  we do not put an emphasis on trying to describe the vertical deformation \DIFdelbegin \DIFdel{as }\DIFdelend \DIFaddbegin \DIFadd{because }\DIFaddend it likely does not have postseismic origins.        

\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=0.8]{Figures/MapViewNearField}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=0.9]{Figures/2016jb013114-p04}
\DIFaddendFL \centering 
\caption{Near-field coseismic and cumulative postseismic displacements over the indicated time periods (black) \DIFdelbeginFL \DIFdelFL{as well as }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and }\DIFaddendFL predicted displacements for our preferred model from Section \ref{sec:FullInversion} (green).  \DIFaddbeginFL \DIFaddFL{The black error ellipses show the 68}\% \DIFaddFL{confidence interval for the observed horizontal displacements.  }\DIFaddendFL Observed vertical \DIFdelbeginFL \DIFdelFL{deformation is }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{displacements are }\DIFaddendFL shown as an interpolated field and predicted vertical displacements are shown within the \DIFaddbeginFL \DIFaddFL{green }\DIFaddendFL circles\DIFdelbeginFL \DIFdelFL{at the base of each vector}\DIFdelendFL .  Note that the interpolant is not well constrained in Mexico where there is no data available.}
\label{fig:NearField}
\end{figure}

\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=0.8]{Figures/MapViewFarField}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=0.9]{Figures/2016jb013114-p05}
\DIFaddendFL \centering 
\caption{Same as Figure \ref{fig:NearField} but for far-field stations.}
\label{fig:FarField}
\end{figure}

The near-field postseismic deformation is notably sustained when compared to the far-field deformation.  Namely, the station in this study which is closest to the El Mayor-Cucapah epicenter, P496, has \DIFdelbegin \DIFdel{been moving at a steady rate }\DIFdelend \DIFaddbegin \DIFadd{a steady postseismic trend }\DIFaddend of ${\sim}1.5$ cm/yr to the south \DIFdelbegin \DIFdel{since }\DIFdelend \DIFaddbegin \DIFadd{after }\DIFaddend about one year\DIFdelbegin \DIFdel{after the El Mayor-Cucapah earthquake}\DIFdelend .  Vertical postseismic deformation in the near-field does display a quadrant pattern which is consistent with the coseismic vertical deformation, suggesting that it is resulting from postseismic processes.  However, the vertical postseismic signal is only apparent for the first year after the earthquake (Figure \ref{fig:NearField}).  As with the far-field deformation, there is a general trend of uplift in the near-field after about one year. 
...
We consider a variety of rheologic models for the lower crust and upper mantle. The simplest rheologic model is to consider them to be effectively elastic and isotropic.  In such case, the rheologic parameters consist of the \DIFaddbegin \DIFadd{reasonably well known }\DIFaddend Lam\'e parameters, $\lambda$ and $\mu$, \DIFdelbegin \DIFdel{which are reasonably well known }\DIFdelend and we use the same values used by \citet{Wei2011} throughout this paper (Table \DIFdelbegin \DIFdel{\ref{tab:MaterialProperties}}\DIFdelend \DIFaddbegin \DIFadd{1}\DIFaddend ).  The only unknown is the distribution of fault slip, which can be \DIFdelbegin \DIFdel{easily }\DIFdelend estimated from postseismic deformation through linear least squares.  \DIFdelbegin \DIFdel{Using }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{Rollins2015}
}%DIFAUXCMD
used }\DIFaddend a subset of the GPS stations considered in this study \DIFdelbegin \DIFdel{, \mbox{%DIFAUXCMD
\citet{Rollins2015}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend found that three years of postseismic deformation following the El Mayor-Cucapah earthquake can be explained with afterslip on the coseismic fault plane without requiring any viscoelastic relaxation. We also perform an elastic slip inversion, but we use GPS stations within a larger radius about the El Mayor-Cucapah epicenter (400 km instead of ${\sim}200$ km). Our forward problem describing predicted postseismic deformation\DIFaddbegin \DIFadd{, }\DIFaddend $u_\mathrm{pred}$\DIFaddbegin \DIFadd{, }\DIFaddend in terms of time dependent fault slip, $s$, is

\begin{equation}\label{eq:ElasticForward}
  u_\mathrm{pred}(x,t) = \int_F s(\xi,t)g(x,\xi)d\xi, 
\end{equation}           
where $F$ denotes the fault and $g(x,\xi)$ is the elastic Green's function describing displacement at surface position $x$ resulting from slip at $\xi$ on the fault.  We estimate coseismic slip and the rate of afterslip \DIFdelbegin \DIFdel{at the discrete time intervals , }\DIFdelend \DIFaddbegin \DIFadd{over the postseismic time intervals }\DIFaddend 0.0-0.125, 0.125-0.25, \DIFaddbegin \DIFadd{0.25-0.5, }\DIFaddend 0.5-1.0, 1.0-2.0, 2.0-3.0, 3.0-4.0, and 4.0-5.0 years\DIFdelbegin \DIFdel{after the earthquake}\DIFdelend .  Each fault segment is discretized into roughly 4 km by 4 km patches \DIFdelbegin \DIFdel{and we estimate a strike-slip and thrust component of slip for each patch. We }\DIFdelend \DIFaddbegin \DIFadd{an we }\DIFaddend impose that the direction of slip and slip rate are within 45$^\circ$ of right-lateral. We also add zeroth-order Tikhonov regularization so that our solution for $s$ satisfies
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=0.8]{Figures/ElasticSlip}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=0.9]{Figures/2016jb013114-p06}
\DIFaddendFL \caption{Coseismic slip and cumulative afterslip over the indicated \DIFaddbeginFL \DIFaddFL{time }\DIFaddendFL intervals \DIFdelbeginFL \DIFdelFL{for }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{when assuming }\DIFaddendFL the \DIFaddbeginFL \DIFaddFL{crust and mantle are }\DIFaddendFL elastic\DIFdelbeginFL \DIFdelFL{model from Section \ref{sec:ElasticInversion}}\DIFdelendFL .  Color indicates the magnitude of slip \DIFdelbeginFL \DIFdelFL{while }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and }\DIFaddendFL arrows indicate the motion of the hanging wall.}
\label{fig:ElasticSlip}
\end{figure}

\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=0.8]{Figures/RecordSectionElastic}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=0.9]{Figures/2016jb013114-p07}
\DIFaddendFL \caption{Scaled radial component of \DIFdelbeginFL \DIFdelFL{observed }\DIFdelendFL postseismic displacements\DIFdelbeginFL \DIFdelFL{$u_\mathrm{obs}$ (black) and displacements predicted by the best fitting elastic model $u_\mathrm{pred}$ (blue)}\DIFdelendFL . Downward motion indicates that the station is moving toward the El Mayor-Cucapah epicenter.  Displacement time series are scaled so that the minimum and maximum observed values lie on the grid lines.  The \DIFaddbeginFL \DIFaddFL{observed postseismic displacements, $u_\mathrm{post}$ are shown in black with gray indicating the }\DIFaddendFL 68\% confidence interval\DIFaddbeginFL \DIFaddFL{.  The displacements predicted by the best fitting elastic model are shown in red.  The blue and green lines are the predicted postseismic displacements }\DIFaddendFL for the \DIFdelbeginFL \DIFdelFL{observed }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{models discussed in Section 3.3. The blue lines show the predicted }\DIFaddendFL displacements \DIFaddbeginFL \DIFaddFL{for the model with a Maxwell viscoelastic lower crust and upper mantle.  The green line shows the predicted displacements for our preferred model, which has a Maxwell viscoelastic lower crust and a Zener viscoelastic upper mantle.  The effective viscosities are the same for both models and }\DIFaddendFL are shown in \DIFdelbeginFL \DIFdelFL{gray}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Figure \ref{fig:EffectiveViscosity}}\DIFaddendFL .}
\DIFdelbeginFL %DIFDELCMD < \label{fig:RecordSectionElastic}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:RecordSectionMain}
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=0.8]{Figures/rheology}
%DIFDELCMD < %%%
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=0.8]{Figures/Cancellation}
%DIFDELCMD < %%%
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=0.8]{Figures/RecordSectionDuration}
%DIFDELCMD < %%%
...
Our \DIFdelbegin \DIFdel{estimated effective viscosities, and corresponding fluidities, are shown in Figure \ref{fig:EffectiveViscosity}.  Although fluidity is rarely used in geophysical literature, eq. (\ref{InitialRate}) is linear with respect to fluidity and so the fluidity indicates the amplitude of the viscoelastic signal coming from each layer.  We also show the 95}%DIFDELCMD < \% %%%
\DIFdel{confidence intervals for estimated viscosities and fluidities but we note that the uncertainties tend to be underestimated due to the added regularization \mbox{%DIFAUXCMD
\citep{Aster2011}
}%DIFAUXCMD
.  A robust feature that we see is that the largest jump in fluidity is at 60 km depth, which is consistent with the range of lithosphere-asthenosphere boundary depths inferred by \mbox{%DIFAUXCMD
\citet{Lekic2011}
}%DIFAUXCMD
. This transitional depth is also consistent with the the viscosity structure required to explain far-field postseismic deformation following the Hector Mine earthquake \mbox{%DIFAUXCMD
\citep{Freed2007a}
}%DIFAUXCMD
...
%DIFDELCMD < \includegraphics[scale=0.8]{Figures/EffectiveViscosity}
%DIFDELCMD < \centering 
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{Effective viscosities and associated fluidities inferred by fitting eq. (\ref{ApproxForward}) to the first 0.8 years of postseismic displacements. 95}%DIFDELCMD < \% %%%
\DIFdel{confidence intervals, estimated from bootstrapping, are indicated by color fields.}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:EffectiveViscosity}
%DIFDELCMD < \end{figure} 
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Our }\DIFdelend initial estimate for coseismic slip and cumulative afterslip over the first 0.8 years after the El Mayor-Cucapah earthquake are shown in Figure \ref{fig:InitialSlip}.  Similar to our elastic slip model from Section \ref{sec:ElasticInversion}, a significant amount of right-lateral and normal coseismic slip is inferred to be on the Sierra Cucapah \DIFdelbegin \DIFdel{fault }\DIFdelend segment. Our coseismic slip solution \DIFaddbegin \DIFadd{on the Sierra Cucapah segment }\DIFaddend is consistent with field studies \citep{Fletcher2014} and the model from \citet{Wei2011}.  \DIFaddbegin \DIFadd{Our inferred slip on the Indiviso fault segment differs from \mbox{%DIFAUXCMD
\citet{Wei2011}
}%DIFAUXCMD
because the GPS data used in this study is not capable of resolving the spatial distribution of fault slip on that segment (Figure S2).  }\DIFaddend The potency of inferred coseismic slip is $3.3\times 10^{9}\ \mathrm{m}^3$, which is also about the same as that inferred from Section \ref{sec:ElasticInversion}. The present inference of afterslip on the Indiviso fault is significantly less than what was found in the Section \ref{sec:ElasticInversion} where we did not account for viscoelasticity. When fault slip is simultaneously estimated with viscosity, the potency of inferred afterslip over the first 0.8 years after the earthquake is $0.85\times 10^9\ \mathrm{m}^3$, compared to $3.5\times10^{9}\ \mathrm{m}^3$ when we assume the crust and upper mantle are elastic.  The significant amount of afterslip inferred on the Indiviso fault \DIFaddbegin \DIFadd{in Section \ref{sec:ElasticInversion} }\DIFaddend seems to be compensating for unmodeled viscoelastic relaxation\DIFdelbegin \DIFdel{at depths greater than $60$ km}\DIFdelend .  The fact that there is still an appreciable amount of afterslip inferred on the Indiviso fault raises the question of whether it is compensating for viscoelastic relaxation that is more localized than what we allow for since we only estimate depth dependent variations in viscosity.

\DIFaddbegin \DIFadd{Our estimated effective viscosities, and corresponding fluidities, are shown in Figure \ref{fig:EffectiveViscosity}.  Although fluidity is rarely used in geophysical literature, eq. (\ref{InitialRate}) is linear with respect to fluidity and so the fluidity indicates the amplitude of the viscoelastic signal coming from each layer.  We use bootstrapping to find the 95}\% \DIFadd{confidence intervals for our estimated effective viscosities which are shown as shaded regions in Figure \ref{fig:EffectiveViscosity}.  It is important to remember that the presented effective viscosities were estimated with a smoothing regularization constraint and so the uncertainties are almost certainly underestimated \mbox{%DIFAUXCMD
\citep{Aster2011}
}%DIFAUXCMD
.  Indeed, many viscosity profiles which are outside of the shown confidence intervals can just as adequately described the first 0.8 years of postseismic deformation. Our solution in Figure \ref{fig:EffectiveViscosity} should be interpreted as the smoothest effective viscosity profile which is capable of describing the data.  This means that any  sharp viscosity transitions will be tapered out in the inversion, which we demonstrate with a synthetic test in Figure S2.  Nonetheless, a robust feature that we see with a variety of choices for $\lambda_s$, $\lambda_\eta$, and $t_\mathrm{bd}$ is that the largest jump in fluidity is at 60 km depth, which is consistent with the range of lithosphere-asthenosphere boundary depths inferred by \mbox{%DIFAUXCMD
\citet{Lekic2011}
}%DIFAUXCMD
. This transitional depth is also consistent with the the viscosity structure required to explain far-field postseismic deformation following the Hector Mine earthquake \mbox{%DIFAUXCMD
\citep{Freed2007a}
...
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=0.8]{Figures/InitialSlip}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=0.9]{Figures/2016jb013114-p11}
\DIFaddendFL \caption{Coseismic slip and afterslip inferred by fitting eq. (\ref{ApproxForward}) to the first 0.8 years of postseismic displacements.}
...
\centering 
\caption{\DIFaddFL{Effective viscosities and associated fluidities inferred by fitting eq. (\ref{ApproxForward}) to the first 0.8 years of postseismic displacements. 95}\% \DIFaddFL{confidence intervals, estimated from bootstrapping, are indicated by shaded regions.}}
\label{fig:EffectiveViscosity}
\end{figure} 

\DIFaddend \subsection{Full Postseismic Inversion}\label{sec:FullInversion} 
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend In the previous section, we used the inverse method from \citet{Hines2016} to constrain the effective viscosity structure required to explain the first 0.8 years of postseismic deformation. In this section, we use these effective viscosities as a prior constraint when searching for models which are capable of describing the available five years of postseismic data, where our forward problem is now eq. (\ref{GeneralForward}) rather than the approximation given by eq. (\ref{ApproxForward}).  We perform a series of fault slip inversions assuming a variety of rheologies for the lower crust and upper mantle which are consistent with our findings from Section \ref{sec:InitialInversion}.  We appraise each model using the mean chi-squared value,
...
Instead of exploring a Burgers rheology mantle, which introduces two new parameters that need to be estimated, the transient viscosity\DIFaddbegin \DIFadd{, }\DIFaddend $\eta_{K}$\DIFaddbegin \DIFadd{, }\DIFaddend and transient shear modulus\DIFaddbegin \DIFadd{, }\DIFaddend $\mu_{K}$, we first consider a Zener rheology for the mantle, which only introduces one \DIFdelbegin \DIFdel{additional unknown }\DIFdelend \DIFaddbegin \DIFadd{unknown model }\DIFaddend parameter, $\mu_{K}$.  We assume that the lower crust still has a Maxwell rheology. The steady-state viscosity in the crust and the transient viscosity in the mantle are set equal to the inferred effective viscosities.  We then estimate the ratio of shear moduli\DIFaddbegin \DIFadd{, }\DIFaddend $\frac{\mu_K}{\mu}$. We compute nine different sets of Green's functions, $f$ and $g$, where we assume values of $\frac{\mu_K}{\mu}$ ranging from 0 to 1. The former being a degenerate case where the Zener model reduces to the above Maxwell model.  We estimate coseismic slip and afterslip for each realization of $\frac{\mu_K}{\mu}$.  \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{We find that a }\DIFaddend shear moduli ratio \DIFdelbegin \DIFdel{that }\DIFdelend \DIFaddbegin \DIFadd{of 0.375 }\DIFaddend yields the best prediction to the observed postseismic displacements \DIFdelbegin \DIFdel{is found to be $0.375$ which produces }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend a misfit of $\bar\chi^2=31.2$ (Figure \ref{fig:ShearModulusRatio}).  The improvement in the Zener model over the Maxwell model can be \DIFdelbegin \DIFdel{clearly }\DIFdelend seen in the fit to the far-field data (Figure \DIFdelbegin \DIFdel{\ref{fig:RecordSection2}) . The }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:RecordSectionMain}) where the }\DIFaddend Zener model does a significantly better job at explaining the transient rate of \DIFdelbegin \DIFdel{far-field }\DIFdelend deformation throughout the five years \DIFdelbegin \DIFdel{.  
}\DIFdelend \DIFaddbegin \DIFadd{considered in this study.  The rheologic parameters for our preferred Zener model are summarized in Table 1.
}\DIFaddend 

\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=0.8]{Figures/RatioMisfit}
%DIFDELCMD < %%%
...
%DIFDELCMD < \includegraphics[scale=0.8]{Figures/RecordSectionFinal}
%DIFDELCMD < \centering 
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
...
%DIFAUXCMD
%DIFDELCMD < \label{fig:RecordSection2}
%DIFDELCMD < \end{figure} 
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Because we are able to adequately describe the available five years of postseismic deformation with a Zener model, we do not find it necessary to explore the parameter space for a more complicated Burgers rheology.  However, since the Zener model is a Burgers model with an infinite steady-state viscosity, we can conclude that any Burgers rheology that has a transient viscosity consistent with that found in Section \ref{sec:InitialInversion} and a steady-state viscosity $\gtrsim10^{20}$ Pa s, which is effectively infinite on the time scale of five years, would also be able to satisfactorily describe the observable postseismic deformation.        

The regularized inference of coseismic slip and afterslip for our preferred Zener model is shown in Figure \ref{fig:FinalSlip}.  The inferred coseismic potency is $3.0\times10^{9}\ \mathrm{m}^3$, equivalent to a \DIFdelbegin \DIFdel{Mw7.26 earthquake, and }\DIFdelend \DIFaddbegin \DIFadd{Mw=7.26 earthquake, where }\DIFaddend most of the slip is shallow and on the Sierra Cucapah fault segment.  The potency of five years of afterslip is $1.1\times10^{9}\ \mathrm{m}^3$. Most of the afterslip in our preferred model occurs within the first year after the earthquake and coincides with the location of our inferred coseismic slip. Inferred afterslip within the first year is accounting for the most rapid near-field transient deformation \DIFaddbegin \DIFadd{(Figure S3)}\DIFaddend .  After one year, afterslip is inferred to be deeper down on the Sierra Cucapah segment\DIFdelbegin \DIFdel{, which is describing much of the }\DIFdelend \DIFaddbegin \DIFadd{.  The }\DIFaddend sustained near-field postseismic deformation \DIFaddbegin \DIFadd{is being explained by this continued afterslip as well as viscoelastic relaxation in the lower crust}\DIFaddend . We emphasize, that the GPS station closest to where we infer afterslip, P496, is still about 30 km away, which is too far for us to conclusively \DIFdelbegin \DIFdel{argue for sustained localized deformation rather than shallow distributed deformation}\DIFdelend \DIFaddbegin \DIFadd{discern deep afterslip from viscoelastic relaxation in the lower crust}\DIFaddend .  The deep afterslip inferred after one year could potentially be \DIFdelbegin \DIFdel{describing deformation resulting from lower crustal flow}\DIFdelend \DIFaddbegin \DIFadd{compensating for an overestimated lower crustal viscosity}\DIFaddend .  To test this, we have modified our preferred model by decreasing the lower crustal viscosity from $5.91\times10^{19}$ Pa s to $1\times10^{19}$ Pa s, which is still consistent with our viscosity inference from Section \ref{sec:InitialInversion}, and \DIFaddbegin \DIFadd{we }\DIFaddend inverted for fault slip.  We find that a model with a weaker lower crust adequately describes the postseismic displacements without any afterslip after one year, while still requiring about the same amount of afterslip over the first year. We do believe that the early \DIFdelbegin \DIFdel{, shallow }\DIFdelend afterslip on the Sierra Cucapah segment is a robust feature in our preferred model, while we are not confident in our inference of later deep afterslip.                   

\DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend postseismic displacements predicted by our preferred \DIFaddbegin \DIFadd{Zener }\DIFaddend model are shown in Figures \ref{fig:NearField}, \ref{fig:FarField} and \DIFdelbegin \DIFdel{\ref{fig:RecordSection2}.  Overall, the trends in the near-field and far-field transient deformation are accurately described.  In particular, the trends in far-field deformation are much better described by our preferred Zener model than either an elastic model or a model with a Maxwell viscoelastic mantle (Figure \ref{fig:RecordSection2}).  There are a few areas where we have notable misfit.  Most of our misfit is for the near-field stations in the Imperial Valley, and we attribute this misfit to our relatively simple fault geometry, which does not account for potential }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:RecordSectionMain}.  The largest misfit occur within the Imperial Valley where there does not appear to be any systematic trend in the residuals.  This suggests that the large errors are due to localized processes such as }\DIFaddend fault slip in the Imperial Valley triggered by the El Mayor-Cucapah earthquake \citep{Wei2011a,Wei2015}.  \DIFdelbegin \DIFdel{In particular, we are unable to model the sustained, rapid rate of deformation at station P496, which suggests that this station could be influenced by a more localized deformation mechanism than is considered in this study}\DIFdelend \DIFaddbegin \DIFadd{We do not see any pattern in the residuals that would suggest a laterally heterogeneous viscosity structure, which has been explored by \mbox{%DIFAUXCMD
\citet{Pollitz2012}
}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citet{Rollins2015}
}%DIFAUXCMD
.  We do notice regional scale seasonal oscillations in the lateral and vertical components of the residuals with an amplitude of 1-2 millimeters.  This is the result of our method for data processing which is not able to completely remove the seasonal signal in the GPS data, which was discussed in Section \ref{sec:Data}}\DIFaddend .  Additionally, we see systematic misfit in the later postseismic period west of the Landers and Hector Mine earthquakes, which may be the result of unmodeled postseismic deformation \DIFdelbegin \DIFdel{resulting from }\DIFdelend \DIFaddbegin \DIFadd{following }\DIFaddend those earthquakes.  Lastly, there are clear discrepancies between the observed and predicted vertical \DIFdelbegin \DIFdel{deformation }\DIFdelend \DIFaddbegin \DIFadd{displacements }\DIFaddend following the first year after the El \DIFdelbegin \DIFdel{Mayor Cucapah }\DIFdelend \DIFaddbegin \DIFadd{Mayor-Cucapah }\DIFaddend earthquake. We observe a broad uplift throughout Southern California \DIFdelbegin \DIFdel{, }\DIFdelend which is inconsistent with any postseismic model.

\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=0.8]{Figures/FinalSlip}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=0.9]{Figures/2016jb013114-p14}
\DIFaddendFL \centering 
\caption{Inferred coseismic slip and afterslip \DIFdelbeginFL \DIFdelFL{when assuming }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{for our preferred model, which has }\DIFaddendFL a Maxwell rheology in the lower crust and a Zener rheology in the upper mantle.  The transient viscosity\DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL $\eta_K$\DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL in the mantle and steady-state viscosity\DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL $\eta_M$\DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL in the crust are set equal to the effective viscosities from Figure \ref{fig:EffectiveViscosity}. We \DIFdelbeginFL \DIFdelFL{set }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{use }\DIFaddendFL $\frac{\mu_K}{\mu}=0.375$ in the upper mantle.}
\label{fig:FinalSlip}
\end{figure}
...
Argus, D.~F., M.~B. Heflin, G.~Peltzer, F.~Cramp{\'{e}}, and F.~H. Webb (2005), {Interseismic strain accumulation and anthropogenic motion in metropolitan Los Angeles}, \textit{Journal of Geophysical Research: Solid Earth}, \DIFdelbegin %DIFDELCMD < \textit{110}(4)%%%
...
Cavali{\'{e}}, O., M.~P. Doin, C.~Lasserre, and P.~Briole (2007), {Ground motion measurement in the Lake Mead area, Nevada, by differential synthetic aperture radar interferometry time series analysis: Probing the lithosphere rheological structure}, \textit{Journal of Geophysical Research: Solid Earth}, \DIFdelbegin %DIFDELCMD < \textit{112}(3)%%%
\DIFdelend \DIFaddbegin \textit{\DIFadd{112}}\DIFadd{(B03403)}\DIFaddend , 1--18, \doi{10.1029/2006JB004344}.

\bibitem[{\textit{Cetin et~al.}(2014)\textit{Cetin, Cakir, Meghraoui, Ergintav, and Akoglu}}]{Cetin2014}
Cetin, E., Z.~Cakir, M.~Meghraoui, S.~Ergintav, and A.~M. Akoglu (2014), {Extent and distribution of aseismic slip on the Ismetpasa segment of the North Anatolian Fault (Turkey) from Persistent Scatterer InSAR}, \textit{Geochemistry, Geophysics, Geosystems}, \textit{15}, \DIFdelbegin %DIFDELCMD < \doi{10.1002/2014GC005307.Received}%%%
\DIFdelend \DIFaddbegin \doi{10.1002/2014GC005307}\DIFaddend .
...
Crittenden, M. (1967), {Viscosity and Finite Strength of the Mantle as Determined from Water and Ice \DIFdelbegin \DIFdel{Loads*}\DIFdelend \DIFaddbegin \DIFadd{Loads}\DIFaddend }, \textit{Geophysical Journal of the Royal Astronomical \DIFdelbegin %DIFDELCMD < {%%%
\DIFdel{\ldots}%DIFDELCMD < }%%%
\DIFdelend \DIFaddbegin \DIFadd{Society}\DIFaddend }, \DIFdelbegin \DIFdel{pp. }\DIFdelend 261--279, \doi{10.1111/j.1365-246X.1967.tb06243.x}.

\bibitem[{\textit{Davis et~al.}(2012)\textit{Davis, Wernicke, and Tamisiea}}]{Davis2012}
Davis, J.~L., B.~P. Wernicke, and M.~E. Tamisiea (2012), {On seasonal signals in geodetic time series}, \textit{Journal of Geophysical Research: Solid Earth}, \DIFdelbegin %DIFDELCMD < \textit{117}(1)%%%
\DIFdelend \DIFaddbegin \textit{\DIFadd{117}}\DIFadd{(B01403)}\DIFaddend , 1--10, \doi{10.1029/2011JB008690}.
...
Gonzalez-Ortega, A., Y.~Fialko, D.~Sandwell, F.~A. Nava-pichardo, J.~Fletcher, J.~Gonzalez-garcia, B.~Lipovsky, M.~Floyd, and G.~Funning (2014), {El Mayor-Cucapah (Mw7.2) earthquake: early near-field postseismic deformation from InSar and GPS observations}, \textit{Journal of Geophysical Research\DIFaddbegin \DIFadd{: Solid Earth}\DIFaddend }, \textit{119}, 1482--1497, \DIFdelbegin %DIFDELCMD < \doi{10.1002/2013JB010193.Received}%%%
\DIFdelend \DIFaddbegin \doi{10.1002/2013JB010193}\DIFaddend .

\bibitem[{\textit{Hauksson et~al.}(2011)\textit{Hauksson, Stock, Hutton, Yang, Vidal-Villegas, and Kanamori}}]{Hauksson2011}
Hauksson, E., J.~Stock, K.~Hutton, W.~Yang, J.~A. Vidal-Villegas, and H.~Kanamori (2011), {The 2010 Mw 7.2 El mayor-cucapah earthquake sequence, Baja California, Mexico and Southernmost California, USA: Active seismotectonics along the Mexican pacific margin}, \textit{Pure and Applied Geophysics}, \DIFdelbegin %DIFDELCMD < \textit{168}(3918)%%%
\DIFdelend \DIFaddbegin \textit{\DIFadd{168}}\DIFaddend , 1255--1277, \doi{10.1007/s00024-010-0209-7}.

\bibitem[{\textit{Hearn}(2003)}]{Hearn2003}
Hearn, E.~H. (2003), {What can GPS data tell us about the dynamics of post-seismic deformation\DIFdelbegin \DIFdel{?}\DIFdelend }, \DIFdelbegin \DIFdel{(2003)}\DIFdelend \DIFaddbegin \textit{\DIFadd{Geophysical Journal International}}\DIFadd{, }\textit{\DIFadd{115}}\DIFaddend , 753--777\DIFaddbegin \DIFadd{, }\doi{10.1111/j.1365-246X.2003.02030.x}\DIFaddend .

\bibitem[{\textit{Hearn et~al.}(2009)\textit{Hearn, McClusky, Ergintav, and Reilinger}}]{Hearn2009} 
Hearn, E.~H., S.~McClusky, S.~Ergintav, and R.~E. Reilinger (2009), {Izmit earthquake postseismic deformation and dynamics of the North Anatolian Fault Zone}, \textit{Journal of Geophysical Research: Solid Earth}, \textit{114}(\DIFdelbegin \DIFdel{August 2008}\DIFdelend \DIFaddbegin \DIFadd{B08405}\DIFaddend ), 1--21, \doi{10.1029/2008JB006026}.

\bibitem[{\textit{Hetland}(2003)}]{Hetland2003}
Hetland, E.~A. (2003), {Postseismic relaxation across the Central Nevada Seismic Belt}, \textit{Journal of Geophysical Research}, \textit{108}(\DIFdelbegin \DIFdel{Figure
  2}\DIFdelend \DIFaddbegin \DIFadd{B8}\DIFaddend ), 1--13, \doi{10.1029/2002JB002257}.

\bibitem[{\textit{Hetland and Zhang}(2014)}]{Hetland2014}
Hetland, E.~A., and G.~Zhang (2014), {Effect of shear zones on post-seismic deformation with application to the 1997 Mw 7.6 manyi earthquake}, \textit{Geophysical Journal International}, \DIFdelbegin \textit{\DIFdel{198}}%DIFAUXCMD
\DIFdel{, 259--269}\DIFdelend \DIFaddbegin \textit{206}(2)\DIFadd{, 1--11}\DIFaddend , \doi{10.1093/gji/ggu127}.
...
Jacobs, A., D.~Sandwell, Y.~Fialko, and L.~Sichoix (2002), {The 1999 (Mw7.1) Hector Mine, California, Earthquake: Near-Field Postseismic Deformation from ERS Interferometry}, \textit{Bulletin of the Seismological Society of America}, \DIFdelbegin \textit{\DIFdel{92}}%DIFAUXCMD
\DIFdel{(May)}\DIFdelend \DIFaddbegin \textit{92}(4)\DIFaddend , 1433--1442.

\bibitem[{\textit{Johnson and Segall}(2004)}]{Johnson2004}
Johnson, K.~M., and P.~Segall (2004), {Viscoelastic earthquake cycle models with deep stress-driven creep along the San Andreas fault system}, \textit{Journal of Geophysical Research\DIFdelbegin \DIFdel{B}\DIFdelend : Solid Earth}, \textit{109}\DIFaddbegin \DIFadd{(B10403)}\DIFaddend , 1--19, \doi{10.1029/2004JB003096}.
...
J{\'{o}}nsson, S., P.~Segall, R.~Pedersen, and G.~Bjornsson (2003), {Post-earthquake ground movements correlated to pore-pressure transients}, \textit{Nature}, \textit{424}\DIFdelbegin \DIFdel{(July)}\DIFdelend , 179--183, \doi{10.1038/nature01758.1.}
...
Kroll, K.~A., E.~S. Cochran, K.~B. Richards-Dinger, and D.~F. Sumy (2013), {Aftershocks of the 2010 Mw 7.2 El Mayor-Cucapah earthquake reveal complex faulting in the Yuha Desert, California}, \textit{Journal of Geophysical Research: Solid Earth}, \textit{118}\DIFdelbegin \DIFdel{(October)}\DIFdelend , 6146--6164, \doi{10.1002/2013JB010529}.

\bibitem[{\textit{Lekic et~al.}(2011)\textit{Lekic, French, and Fischer}}]{Lekic2011}
Lekic, V., S.~W. French, and K.~M. Fischer (2011), {Lithospheric Thinning Beneath Rifted Regions of Southern California}, \textit{Science} \DIFdelbegin \DIFdel{,
  }\DIFdelend \textit{334}\DIFdelbegin \DIFdel{(November)}\DIFdelend , 783--787.

\bibitem[{\textit{Liu et~al.}(1976)\textit{Liu, Anderson, and Kanamori}}]{Liu1976}
Liu, H.-P., D.~L. Anderson, and H.~Kanamori (1976), {Velocity dispersion due to anelasticity; implications for seismology and mantle composition} \DIFdelbegin \DIFdel{,
  }\DIFdelend \DIFaddbegin \textit{\DIFadd{Geophysical Journal of the Royal Astronomical Society}}\DIFadd{, }\DIFaddend \textit{\DIFdelbegin \DIFdel{Geophysical Journal International}\DIFdelend \DIFaddbegin \DIFadd{47}\DIFaddend }, \DIFdelbegin %DIFDELCMD < \textit{47}(1)%%%
\DIFdel{, }\DIFdelend 41--58, \doi{10.1111/j.1365-246X.1976.tb01261.x}.

\bibitem[{\textit{Lundgren et~al.}(2009)\textit{Lundgren, Hetland, Liu, and Fielding}}]{Lundgren2009}
Lundgren, P., E.~A. Hetland, Z.~Liu, and E.~J. Fielding (2009), {Southern San Andreas-San Jacinto fault system slip rates estimated from earthquake cycle models constrained by GPS and interferometric synthetic aperture radar observations}, \textit{Journal of Geophysical Research: Solid Earth} \DIFdelbegin \DIFdel{,
  }%DIFDELCMD < \textit{114}(2)%%%
\DIFdelend \DIFaddbegin \textit{\DIFadd{114}}\DIFadd{(B02403)}\DIFaddend , 1--18, \doi{10.1029/2008JB005996}.

\bibitem[{\textit{McGuire and Segall}(2003)}]{McGuire2003}
McGuire, J.~J., and P.~Segall (2003), {Imaging of aseismic fault slip transients recorded by dense geodetic networks}, \textit{Geophysical Journal International}, \DIFdelbegin %DIFDELCMD < \textit{155}(3)%%%
\DIFdelend \DIFaddbegin \textit{\DIFadd{155}}\DIFaddend , 778--788, \doi{10.1111/j.1365-246X.2003.02022.x}.

\bibitem[{\textit{Meade and Hager}(2005)}]{Meade2005}
Meade, B.~J., and B.~H. Hager (2005), {Block models of crustal motion in southern California constrained by GPS measurements}, \textit{Journal of Geophysical Research\DIFdelbegin \DIFdel{B}\DIFdelend : Solid Earth}, \textit{110}\DIFaddbegin \DIFadd{(B03403)}\DIFaddend , 1--19, \doi{10.1029/2004JB003209}.

\bibitem[{\textit{Murray and Segall}(2005)}]{Murray2005}
Murray, J.~R., and P.~Segall (2005), {Spatiotemporal evolution of a transient slip event on the San Andreas fault near Parkfield, California}, \textit{Journal of Geophysical Research: Solid Earth}, \DIFdelbegin %DIFDELCMD < \textit{110}(9)%%%
\DIFdelend \DIFaddbegin \textit{\DIFadd{110}}\DIFadd{(B09407)}\DIFaddend , 1--12, \doi{10.1029/2005JB003651}.
...
Oskin, M.~E., J.~R. Arrowsmith, A.~{Hinojosa Corona}, A.~J. Elliott, J.~M. Fletcher, E.~J. Fielding, P.~O. Gold, J.~J. {Gonzalez Garcia}, K.~W. Hudnut, J.~Liu-Zeng, and O.~J. Teran (2012), {Near-field deformation from the El Mayor-Cucapah earthquake revealed by differential LIDAR}, \textit{Science}, \DIFdelbegin %DIFDELCMD < \textit{335}(6069)%%%
\DIFdelend \DIFaddbegin \textit{\DIFadd{335}}\DIFaddend , 702--705, \doi{10.1126/science.1213778}.

\bibitem[{\textit{Pollitz et~al.}(2001)\textit{Pollitz, Wicks, and Thatcher}}]{Pollitz2001}
Pollitz, F., C.~Wicks, and W.~Thatcher (2001), {Mantle Flow Beneath a Continental Strike-Slip Fault: Postseismic Deformation After the 1999 Hector Mine Earthquake}, \textit{Science}, \DIFdelbegin %DIFDELCMD < \textit{1814}(2001)%%%
\DIFdelend \DIFaddbegin \textit{\DIFadd{293}}\DIFaddend , 1814--1818, \doi{10.1126/science.1061361}.
...
Pollitz, F.~F., G.~Peltzer, and R.~B{\"{u}}rgmann (2000), {Mobility of continental mantle: Evidence from postseismic geodetic observations following the 1992 Landers earthquake}, \textit{Journal of Geophysical Research}, \DIFdelbegin %DIFDELCMD < \textit{105}(1999)%%%
\DIFdel{, 8035}\DIFdelend \DIFaddbegin \textit{\DIFadd{105}}\DIFadd{(B4), 8035--8054}\DIFaddend , \doi{10.1029/1999JB900380}.
...
\DIFaddbegin \bibitem[{\textit{Pollitz}(2015)}]{Pollitz2015}
\DIFadd{Pollitz, F.~F. (2015). }{\DIFadd{Postearthquake relaxation evidence for laterally variable viscoelastic structure and water content in the Southern California mantle}}\DIFadd{, }\textit{\DIFadd{Journal of Geophysical Research: Solid Earth}}\DIFadd{, }\textit{\DIFadd{120}}\DIFadd{, 2672--2696, }\doi{10.1002/2014JB011603}\DIFadd{.
}

\DIFaddend \bibitem[{\textit{Rauch et~al.}(1965)\textit{Rauch, Tung, and Striebel}}]{Rauch1965}
...
Rollins, C., S.~Barbot, and J.-P. Avouac (2015), {Postseismic Deformation Following the 2010 M7.2 El Mayor-Cucapah Earthquake: Observations, Kinematic Inversions, and Dynamic Models}, \textit{Pure and Applied Geophysics}, \DIFaddbegin \textit{172}(5)\DIFadd{, 1305-1358, }\DIFaddend \doi{10.1007/s00024-014-1005-6}.

\bibitem[{\textit{Savage}(1990)}]{Savage1990}
Savage, J.~C. (1990), {Equivalent strike-slip earthquake cycles in half-space and lithosphere-asthenosphere earth models}, \textit{Journal of Geophysical Research}, \textit{95}\DIFdelbegin \DIFdel{, 4873}\DIFdelend \DIFaddbegin \DIFadd{(B4), 4873--4879}\DIFaddend , \doi{10.1029/JB095iB04p04873}.

\bibitem[{\textit{Savage and Svarc}(2009)}]{Savage2009}
Savage, J.~C., and J.~L. Svarc (2009), {Postseismic relaxation following the 1992 \DIFdelbegin \DIFdel{M 7.3 }\DIFdelend \DIFaddbegin \DIFadd{M7.3 }\DIFaddend Landers and 1999 \DIFdelbegin \DIFdel{M 7.1 }\DIFdelend \DIFaddbegin \DIFadd{M7.1 }\DIFaddend Hector Mine earthquakes, southern California}, \textit{Journal of Geophysical Research}, \textit{114}(B01401), \doi{10.1029/2008JB005938}.

\bibitem[{\textit{Savage et~al.}(2005)\textit{Savage, Svarc, and Yu}}]{Savage2005a}
Savage, J.~C., J.~L. Svarc, and S.~B. Yu (2005), {Postseismic relaxation and transient creep}, \textit{Journal of Geophysical Research: Solid Earth}, \textit{110}\DIFaddbegin \DIFadd{(B11402)}\DIFaddend , 1--14, \doi{10.1029/2005JB003687}.
...
Spinler, J.~C., R.~A. Bennett, C.~Walls, L.~Shawn, and J.~J.~G. Garcia (2015), {Assessing long-term postseismic deformation following the M7.2 4 April 2010, El Mayor-Cucapah earthquake with implications for lithospheric rheology in the Salton Trough}, \textit{Journal of Geophysical Research: Solid Earth}, \textit{120}, 3664--3679, \DIFdelbegin %DIFDELCMD < \doi{10.1002/2014JB011613.Received}%%%
\DIFdelend \DIFaddbegin \doi{10.1002/2014JB011613}\DIFaddend .

\bibitem[{\textit{Wei et~al.}(2011{\natexlab{a}})\textit{Wei, Sandwell, Fialko, and Bilham}}]{Wei2011a}
Wei, M., D.~Sandwell, Y.~Fialko, and R.~Bilham (2011{\natexlab{a}}), {Slip on faults in the Imperial Valley triggered by the 4 April 2010 Mw 7.2 El Mayor-Cucapah earthquake revealed by InSAR}, \textit{Geophysical Research Letters}, \textit{38}(\DIFdelbegin \DIFdel{January}\DIFdelend \DIFaddbegin \DIFadd{L01308}\DIFaddend ), 1--6 \DIFdelbegin \DIFdel{, }\DIFdelend \doi{10.1029/2010GL045235}.
...
Wei, S., E.~Fielding, S.~Leprince, A.~Sladen, J.-P. Avouac, D.~Helmberger, E.~Hauksson, R.~Chu, M.~Simons, K.~Hudnut, T.~Herring, and R.~Briggs\DIFaddbegin \DIFadd{,}\DIFaddend (2011{\natexlab{b}}), {Superficial simplicity of the 2010 El Mayor–Cucapah earthquake of Baja California in Mexico}, \textit{Nature Geoscience}, \DIFdelbegin %DIFDELCMD < \textit{4}(9)%%%
\DIFdelend \DIFaddbegin \textit{\DIFadd{4}}\DIFaddend , 615--618, \doi{10.1038/ngeo1213}.
...
%DIF < %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DIF <  Track Changes:
%DIF <  To add words, \added{<word added>}
%DIF <  To delete words, \deleted{<word deleted>}
%DIF <  To replace words, \replace{<word to be replaced>}{<replacement word>}
%DIF <  To explain why change was made: \explain{<explanation>} This will put
%DIF <  a comment into the right margin.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DIF <  At the end of the document, use \listofchanges, which will list the
%DIF <  changes and the page and line number where the change was made.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  When final version, \listofchanges will not produce anything,
%DIF <  \added{<word or words>} word will be printed, \deleted{<word or words} will take away the word,
%DIF <  \replaced{<delete this word>}{<replace with this word>} will print only the replacement word.
%DIF <   In the final version, \explain will not print anything.
%DIF < %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFdelend 
