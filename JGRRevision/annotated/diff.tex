...
%DIF ADD new.tex   Sat Jul  2 00:54:36 2016
\draftfalse
...
\item \DIFdelbegin \DIFdel{A }\DIFdelend \DIFaddbegin \DIFadd{Far-field postseismic deformation can be explained with a }\DIFaddend Zener or Burgers \DIFdelbegin \DIFdel{upper mantle can explain far-field deformation while near-field transience can be described with afterslip
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=0.9]{Figures/FilterP496}
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=0.9]{Figures/MapViewNearField}
\DIFaddendFL \centering 
\caption{Near-field coseismic and cumulative postseismic displacements over the indicated time periods (black) \DIFdelbeginFL \DIFdelFL{as well as }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and }\DIFaddendFL predicted displacements for our preferred model from Section \ref{sec:FullInversion} (green).  Observed vertical \DIFdelbeginFL \DIFdelFL{deformation is }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{displacements are }\DIFaddendFL shown as an interpolated field and predicted vertical displacements are shown within the \DIFaddbeginFL \DIFaddFL{green }\DIFaddendFL circles\DIFdelbeginFL \DIFdelFL{at the base of each vector}\DIFdelendFL .  Note that the interpolant is not well constrained in Mexico where there is no data available.}
\label{fig:NearField}
\end{figure}
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=0.9]{Figures/MapViewFarField}
\DIFaddendFL \centering 
...
We consider a variety of rheologic models for the lower crust and upper mantle. The simplest rheologic model is to consider them to be effectively elastic and isotropic.  In such case, the rheologic parameters consist of the Lam\'e parameters, $\lambda$ and $\mu$, which are reasonably well known and we use the same values used by \citet{Wei2011} throughout this paper (Table \DIFdelbegin \DIFdel{\ref{tab:MaterialProperties}}\DIFdelend \DIFaddbegin \DIFadd{1}\DIFaddend ).  The only unknown is the distribution of fault slip, which can be easily estimated from postseismic deformation through linear least squares.  \DIFdelbegin \DIFdel{Using }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{Rollins2015}
}%DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=0.9]{Figures/rheology}
\DIFaddendFL \centering 
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=0.9]{Figures/Cancellation}
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=0.9]{Figures/RecordSectionDuration}
\DIFaddendFL \centering 
...
.  Indeed, many viscosity profiles which are outside of the shown confidence intervals can just as adequately described the first 0.8 years of postseismic deformation. Our solution in Figure \ref{fig:EffectiveViscosity} should be interpreted as the smoothest effective viscosity profile which is capable of describing the data.  This means that any  sharp viscosity transitions will be tapered out in the inversion, which we demonstrate with a synthetic test in Figure S2.  Nonetheless, a robust feature that we see is that the largest jump in fluidity is at 60 km depth, which is consistent with the range of lithosphere-asthenosphere boundary depths inferred by \mbox{%DIFAUXCMD
\citet{Lekic2011}
}%DIFAUXCMD
. This transitional depth is also consistent with the the viscosity structure required to explain far-field postseismic deformation following the Hector Mine earthquake \mbox{%DIFAUXCMD
\citep{Freed2007a}
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=0.9]{Figures/InitialSlip}
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=0.9]{Figures/RatioMisfit}
\DIFaddendFL \centering 
...
Johnson, K.~M., and P.~Segall (2004), {Viscoelastic earthquake cycle models
  ...
  \textit{Journal of Geophysical Research\DIFdelbegin \DIFdel{B}\DIFdelend : Solid Earth}, \textit{109}, 1--19,
...
\bibitem[{\textit{Pollitz et~al.}(2012)\textit{Pollitz, B{\"{u}}rgmann, and
  ...
  1--17, \doi{10.1029/2012GC004139}\DIFaddbegin \DIFadd{.
}

\bibitem[{\textit{Pollitz}(2015)}]{Pollitz2015}
\DIFadd{Pollitz, F.~F. (2015). }{\DIFadd{Postearthquake relaxation evidence for laterally variable viscoelastic structure and water content in the Southern California mantle}}\DIFadd{, }\textit{\DIFadd{Journal of Geophysical Research: Solid Earth}}\DIFadd{, }\textit{\DIFadd{120}}\DIFadd{,2672--2696, }\doi{10.1002/2014JB011603}\DIFaddend .
